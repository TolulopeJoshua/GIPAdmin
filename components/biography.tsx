...
const Biography = ({biography}: {biography: biography}) => {
    ...
    const onDelete = (e: any) => {
        ...
        if (confirm('Sure to delete?')) {
          ...
          const url = proxy + `/book/${compBiography._id}`;
          axios.delete(url, { headers: {'Authorization': `Bearer ${auth.token}`}})
            .then(function (response) {
              dispatch(docsActions.deleteBiography(response.data));
              toast.dismiss()
    ...
    <form className='w-5/12 px-1 h-full flex flex-col'>
        ...
        <div contentEditable={textMode} id={`${biography._id}_text`} className={'w-7/12 p-1 pb-8 h-full overflow-y-auto absolute right-0 top-0 scrollbar text-sm text-justify whitespace-pre-line ' + (textMode ? 'bg-slate-200 text-black' : 'bg-white/30')}></div>
